...
start_webcam_overlay() {
  ...
  ffplay -f v4l2 $video_size_arg -input_format mjpeg -framerate 30 /dev/video0 \
...
stop_screenrecording() {
  ...
  # Wait a maximum of 30 seconds to finish before hard killing
  # GPU encoding can take time to finalize the video file
  local count=0
  while pgrep -f "gpu-screen-recorder" >/dev/null && [ $count -lt 300 ]; do
    sleep 0.1
    count=$((count + 1))
  done
