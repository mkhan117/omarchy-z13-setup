...
# Run user CPU overclock script if it exists
if [[ -x "$HOME/oc-cpu-v2.sh" ]]; then
    "$HOME/oc-cpu-v2.sh" 2>/dev/null && log "User OC script applied" || log "User OC script failed"
fi

# Start EasyEffects in background for audio processing
if command -v easyeffects >/dev/null 2>&1; then
    WAYLAND_DISPLAY=gamescope-0 easyeffects --gapplication-service &
    log "EasyEffects started"
fi
...
SUDOERS_SWITCH
  ...
  if [[ ! -f "$hypr_bindings_conf" ]]; then
    ...
    warn "You can manually add: bindd = SUPER SHIFT, F5, Gaming Mode, exec, /usr/local/bin/switch-to-gaming"
  else
...
bindd = SUPER SHIFT, F5, Gaming Mode, exec, /usr/local/bin/switch-to-gaming
HYPR_GAMING
